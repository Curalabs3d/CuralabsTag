...
export default function AdminPanel() {
  const { token } = useAuth();
  const [view, setView] = useState('empresas'); // empresas | billing
  const [overview, setOverview] = useState(null);
  const [tenants, setTenants] = useState([]);
  const [filter, setFilter] = useState('ALL');
  const [loading, setLoading] = useState(true);
  const [busyId, setBusyId] = useState(null);
  const [moduleModalTenant, setModuleModalTenant] = useState(null);
  ...
  useEffect(() => { if (view === 'empresas') load(); }, [load, view]);
  ...
  return (
    ...
    >
      <div className="mb-6 flex items-center gap-1 border-b border-white/5">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px
              ${view === v.id ? 'border-accent text-white' : 'border-transparent text-white/40 hover:text-white/70'}`}
          >
      ...
      {moduleModalTenant && (
        <TenantModuleOverridesModal tenant={moduleModalTenant} onClose={() => setModuleModalTenant(null)} />
      )}
    </AppShell>
  );
}
